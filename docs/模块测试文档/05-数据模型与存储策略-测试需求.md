...
- 书籍目录模型（BookRoot、ActiveBookRoot）
- 文档模型（Document、Section、Paragraph）
- 原文片段模型（SourceSnippet）
- 知识点模型（KnowledgePoint、KnowledgeRelation、RelationType）
...
- 枚举类型转换器（ExerciseTypeConverter、ParagraphType）
...
| 模型类型 | 用途 | 预期内容特征 |
|----------|------|--------------|
| BookRoot | 书籍目录配置 | 包含 Id、Name、Path、ReferenceDirNames 等字段 |
| Document | 主书内容文档 | 包含 DocId、Path、Title、Sections 等字段 |
| SourceSnippet | 原文片段 | 包含 SnippetId、FilePath、Content、StartLine、EndLine |
| KnowledgePoint | 知识点 | 包含 KpId、Title、ChapterPath、Importance、SnippetIds |
| KnowledgeSystem | 知识系统容器 | 包含 BookRootId、KnowledgePoints、Snippets、Tree |
| LearningPack | 学习内容包 | 包含 KpId、Summary、Levels、SnippetIds |
...
| 枚举类型 | 测试值 |
|----------|--------|
| ParagraphType | Text、Code、Quote、List |
...
### 3.1 书籍目录模型测试（BookRoot）

#### TC-BR-001: BookRoot 基础创建

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 创建 BookRoot 实例，设置 Id、Name、Path |
| 预期输出 | 对象字段正确初始化 |
| 验证点 | Id、Name、Path 属性正确设置 |

#### TC-BR-002: BookRoot 默认值验证

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 创建 BookRoot 实例，不设置可选字段 |
| 预期输出 | 可选字段使用默认值 |
| 验证点 | Enabled=true, ReferenceDirNames 包含默认值 |

#### TC-BR-003: ActiveBookRoot 创建

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 创建 ActiveBookRoot 实例，设置 ActiveBookRootId |
| 预期输出 | ActiveBookRootId 属性正确设置 |
| 验证点 | 可为 null（未激活状态） |

---

### 3.2 文档模型测试（Document/Section/Paragraph）
...
#### TC-DOC-003: Paragraph 行号范围

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 创建 Paragraph，设置 StartLine、EndLine |
| 预期输出 | 行号范围正确存储 |
| 验证点 | StartLine <= EndLine |

#### TC-DOC-004: ParagraphType 枚举值

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 设置 Paragraph.Type 为各枚举值 |
| 预期输出 | 正确解析为对应枚举 |
| 验证点 | Text、Code、Quote、List 均可正确设置 |

---

### 3.3 原文片段模型测试（SourceSnippet）

#### TC-SS-001: SourceSnippet 基础创建

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 创建 SourceSnippet 实例 |
| 预期输出 | 所有字段正确初始化 |
| 验证点 | SnippetId、Content、StartLine、EndLine 正确 |

#### TC-SS-002: SourceSnippet 文件追溯

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 设置 FilePath、HeadingPath、StartLine、EndLine |
| 预期输出 | 可完整追溯原文位置 |
| 验证点 | 路径和行号信息完整保留 |

#### TC-SS-003: SourceSnippet 原文引用字段

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 设置 DocId、FilePath、HeadingPath |
| 预期输出 | 原文引用字段正确存储 |
| 验证点 | 各字段可正确序列化和反序列化 |
...
| 预期输出 | 对象字段正确初始化 |
| 验证点 | Aliases、SnippetIds、Relations 初始化为空列表 |
...
#### TC-KP-004: KnowledgeRelation 关联类型

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 创建 KnowledgeRelation，设置各 RelationType |
| 预期输出 | 关联类型正确存储 |
| 验证点 | Prerequisite、Comparison、Contains、Related 可正确设置 |

#### TC-KP-005: KnowledgePoint 原文片段关联

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 设置 SnippetIds 包含多个 ID |
| 预期输出 | 片段列表正确存储 |
| 验证点 | 可包含至少 1 个片段 ID |
...
| 预期输出 | 对象字段正确初始化 |
| 验证点 | KnowledgePoints、Snippets 初始化为空 |

#### TC-KS-002: KnowledgeSystem 片段字典

| 项目 | 内容 |
|------|------|
| 前置条件 | 无 |
| 输入 | 向 Snippets 字典添加 SourceSnippet |
| 预期输出 | 字典正确存储 |
| 验证点 | 可通过 SnippetId 检索 |
...
| 输入 | 创建 Exercise 实例 |
| 预期输出 | 对象字段正确初始化 |
| 验证点 | Options、EvidenceSnippetIds、KeyPoints 初始化为空 |
...
| 预期输出 | 反馈结构正确 |
| 验证点 | IsCorrect 可为 null（填空/简答），CoveredPoints、MissingPoints 为列表 |
...
| 输入 | 序列化为 JSON |
| 预期输出 | JSON 包含 BookRootId、KnowledgePoints、Snippets、Tree |
| 验证点 | 树结构正确序列化 |
