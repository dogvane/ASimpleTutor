...
2. **数据生成阶段**：
   ...
   - 保存幻灯片卡片到文件：`saves/{BookId}/slide-cards/kp_*.json`。
   - 保存 TTS口播说明到文件：`saves/{BookId}/tts-scripts/kp_*.txt`。
...
2. **知识产品数据加载**：
   ...
   - 从文件加载幻灯片卡片：`saves/{BookId}/slide-cards/kp_*.json`。
   - 从文件加载 TTS口播说明：`saves/{BookId}/tts-scripts/kp_*.txt`。
...
- **幻灯片卡片**：基于知识点信息和知识图谱生成。
- **TTS口播说明**：基于幻灯片卡片内容生成，与幻灯片关联。
