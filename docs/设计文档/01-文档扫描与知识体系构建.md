...
2. **概念提取 (Concept Extraction)**：
    - 输入：文档片段内容。
    - 为每个叶子节点章节调用 `LLMService` 提取知识点，包含 `Title`, `Type`, `Importance`, `DocId`, `ChapterPath` 等属性。
...
5. **构建知识图谱**：完成知识点构建后，自动基于知识点间的语义关系构建知识图谱。
   - **节点构建**：将知识点转换为知识图谱节点，保留其类型、重要性、章节路径等属性。
   - **关系计算**：自动计算节点间的关系权重，构建语义关联网络（包含、依赖、相关、对比等）。
   - **可视化支持**：为知识图谱提供基础的可视化布局支持，包括节点位置、大小和颜色等。
6. **返回结果**：返回 (KnowledgeSystem, List<Document>) 元组给调用方。
7. **知识系统保存**：调用方使用 KnowledgeSystemStore 将构建好的知识系统保存到文件系统。
...
- `KnowledgeGraphNode`：知识图谱节点，对应一个知识点。
- `KnowledgeGraphEdge`：知识图谱边，表示两个知识点间的关系。

## 7. 核心服务

- **KnowledgeBuilder**：负责协调文档扫描、知识点提取、学习内容生成、知识树构建和知识图谱构建的主流程。
...
- **IKnowledgeGraphBuilder**：负责将知识点转换为知识图谱结构，包括节点构建、关系计算和查询功能。

## 8. 调用流程

1. **初始化**：创建 `KnowledgeBuilder` 实例，注入 `IScannerService`、`ILLMService`、`IKnowledgeGraphBuilder` 和 `ILogger<KnowledgeBuilder>`。
...
7. **构建知识图谱**：`KnowledgeBuilder` 调用 `IKnowledgeGraphBuilder.BuildAsync` 构建知识图谱。
8. **返回结果**：`KnowledgeBuilder` 返回 `(KnowledgeSystem, List<Document>)` 元组给调用方。
9. **保存**：调用方使用 `KnowledgeSystemStore.SaveAsync` 保存知识系统到文件系统。
