# 幻灯片 API 接口规范

## 核心设计理念

**直接复用知识点现有内容**，无需调用 LLM 重新生成。

每个知识点（KnowledgePoint）包含：
- `Summary` - 精要速览（定义、关键点、误区）
- `Levels` - 层次化内容（L1/L2/L3）
- `SnippetIds` - 原文片段引用
- `Relations` - 知识点关联关系

幻灯片（Slides）直接将上述内容按类型组织返回给前端渲染。

---

## 1. 获取知识点的幻灯片内容

### 请求

**Endpoint**: `GET /api/v1/knowledge-points/slides`

**Query Parameters**:

| 参数 | 类型 | 必填 | 说明 |
|-----|------|-----|------|
| kpId | string | 是 | 知识点 ID |

**示例请求**:
```http
GET /api/v1/knowledge-points/slides?kpId=kp_001
```

### 响应

**成功响应** (200 OK):

```json
{
  "schemaVersion": "1.0",
  "kpId": "kp_001",
  "title": "神经元模型",
  "kpType": "concept",
  "slides": [
    {
      "slideId": "kp_001_cover",
      "type": "cover",
      "order": 1,
      "title": "神经元模型",
      "subtitle": "concept",
      "content": "# 概述\n\n*第2章 > 2.1节*\n\n## 定义\n\n神经元是神经网络的基本处理单元，它接收输入信号，进行加权求和，然后通过激活函数产生输出。\n\n## 核心要点\n\n- 接收多个输入信号\n- 加权求和与偏置\n- 激活函数非线性变换\n\n## 常见误区\n\n- ⚠️ 神经元不等于大脑中的神经元\n- ⚠️ 权重不是固定值，需要通过学习获得",
      "kpLinks": [
        {
          "text": "激活函数",
          "targetKpId": "kp_002",
          "relationship": "related"
        }
      ],
      "config": {
        "allowSkip": true,
        "requireComplete": false,
        "estimatedTime": 45
      }
    },
    {
      "slideId": "kp_001_explanation",
      "type": "explanation",
      "order": 2,
      "title": "L1 概览",
      "content": "## L1 概览\n\n神经元的数学表达如下：\n\n```\ny = f(Σ wᵢxᵢ + b)\n```\n\n其中：\n- xᵢ 是输入信号\n- wᵢ 是对应权重\n- b 是偏置\n- f 是激活函数\n- y 是输出",
      "kpLinks": [],
      "config": {
        "allowSkip": true,
        "requireComplete": false,
        "estimatedTime": 90
      }
    },
    {
      "slideId": "kp_001_detail",
      "type": "detail",
      "order": 3,
      "title": "L2 详细",
      "content": "## L2 详细\n\n### 数学原理\n\n神经元的输出计算分为三个步骤：\n\n1. **加权求和**：将每个输入乘以对应权重后相加\n2. **偏置叠加**：加上偏置项调整激活阈值\n3. **非线性变换**：通过激活函数引入非线性\n\n### 常用激活函数\n\n| 函数 | 公式 | 特点 |\n|------|------|------|\n| Sigmoid | σ(x) = 1/(1+e⁻ˣ) | 输出(0,1)，适合概率 |\n| ReLU | max(0,x) | 计算简单，缓解梯度消失 |\n| Tanh | tanh(x) | 输出(-1,1)，零中心 |",
      "kpLinks": [],
      "config": {
        "allowSkip": true,
        "requireComplete": false,
        "estimatedTime": 120
      }
    },
    {
      "slideId": "kp_001_deepdive",
      "type": "deepDive",
      "order": 4,
      "title": "L3 深入",
      "content": "## L3 深入探讨\n\n### 为什么需要激活函数？\n\n如果没有激活函数，无论网络有多少层，最终都可以简化为线性变换：\n\n```\ny = W₃(W₂(W₁x + b₁) + b₂) + b₃ = W'x + b'\n```\n\n激活函数引入的非线性是神经网络能够学习复杂模式的关键。\n\n### 梯度消失问题\n\nSigmoid/Tanh 函数在两端导数趋近于0，导致深层网络难以训练。ReLU 通过正区间恒定导数缓解了这个问题。",
      "kpLinks": [],
      "config": {
        "allowSkip": true,
        "requireComplete": false,
        "estimatedTime": 150
      }
    },
    {
      "slideId": "kp_001_source",
      "type": "source",
      "order": 5,
      "title": "原文对照",
      "content": "## 原文来源\n\n### 📄 neural-network.md\n\n**位置**: 第2章 > 2.1节 (行 15-20)\n\n> 神经元是神经网络的基本处理单元，它接收输入信号，\n> 进行加权求和，然后通过激活函数产生输出。\n\n---\n\n### 📄 deep-learning-basics.md\n\n**位置**: 第3章 > 激活函数 (行 45-52)\n\n> 激活函数引入非线性变换，是神经网络能够学习复杂\n> 模式的关键。常用的激活函数包括 Sigmoid、ReLU、Tanh 等。\n\n---",
      "kpLinks": [],
      "config": {
        "allowSkip": true,
        "requireComplete": false,
        "estimatedTime": 60
      }
    },
    {
      "slideId": "kp_001_relations",
      "type": "relations",
      "order": 6,
      "title": "知识关联",
      "content": "## 知识关联\n\n### 🔸 前置知识\n\n- [[kp_000]]\n  - 线性代数基础\n\n### 🔗 相关知识\n\n- [[kp_002]]\n  - 激活函数详解\n- [[kp_003]]\n  - 感知机模型\n\n### ⚖️ 对比学习\n\n- [[kp_004]]\n  - SVM 与神经网络对比",
      "kpLinks": [
        {
          "text": "kp_002",
          "targetKpId": "kp_002",
          "relationship": "related"
        },
        {
          "text": "kp_003",
          "targetKpId": "kp_003",
          "relationship": "related"
        },
        {
          "text": "kp_004",
          "targetKpId": "kp_004",
          "relationship": "contrast"
        }
      ],
      "config": {
        "allowSkip": true,
        "requireComplete": false,
        "estimatedTime": 45
      }
    }
  ],
  "meta": {
    "totalSlides": 6,
    "estimatedTime": 510,
    "difficulty": "intermediate",
    "generatedAt": "2025-01-15T10:30:00Z"
  }
}
```

---

## 数据结构说明

### SlideType 枚举

| 类型值 | 说明 | 内容来源 |
|--------|------|----------|
| `cover` | 封面/导言 | `Summary` (Definition, KeyPoints, Pitfalls) |
| `explanation` | 概念解释 | `Levels[0]` (L1) |
| `detail` | 详细内容 | `Levels[1]` (L2) |
| `deepDive` | 深入探讨 | `Levels[2]` (L3) |
| `source` | 原文对照 | `SnippetIds` → Snippets |
| `quiz` | 随堂测验 | `Exercises` (如有) |
| `relations` | 知识关联 | `Relations` |
| `summary` | 总结回顾 | 可选 |

### 字段说明

| 字段 | 类型 | 说明 |
|------|------|------|
| `slideId` | string | 幻灯片唯一标识，格式 `{kpId}_{type}` |
| `type` | SlideTypeDto | 幻灯片类型 |
| `order` | number | 排序序号（从1开始） |
| `title` | string | 幻灯片标题 |
| `subtitle` | string? | 可选的副标题 |
| `content` | string | Markdown 格式的内容，前端根据 type 渲染 |
| `kpLinks` | array | 知识点链接列表 |
| `config.allowSkip` | boolean | 是否允许跳过 |
| `config.requireComplete` | boolean | 是否必须完成才能继续 |
| `config.estimatedTime` | number | 预计阅读时长（秒） |

---

## 幻灯片内容渲染指南（前端）

### Cover 类型

**渲染建议**：
- 标题使用大号居中显示
- 副标题（subtitle）显示知识点类型
- 定义部分使用引用块样式
- 关键点使用列表，每项一个图标
- 误区使用警告图标

```markdown
# 概述

*第2章 > 2.1节*

## 定义
神经元是神经网络的基本处理单元...

## 核心要点
- 接收多个输入信号
- 加权求和与偏置
- 激活函数非线性变换

## 常见误区
- ⚠️ 神经元不等于大脑中的神经元
```

### Explanation / Detail / DeepDive 类型

**渲染建议**：
- 使用标准 Markdown 渲染
- 代码块使用语法高亮
- 表格使用表格样式
- 公式可使用 KaTeX/MathJax 渲染

```markdown
## L2 详细

### 数学原理

神经元的输出计算分为三个步骤：

1. **加权求和**：将每个输入乘以对应权重后相加
2. **偏置叠加**：加上偏置项调整激活阈值
3. **非线性变换**：通过激活函数引入非线性

### 常用激活函数

| 函数 | 公式 | 特点 |
|------|------|------|
| Sigmoid | σ(x) = 1/(1+e⁻ˣ) | 输出(0,1)，适合概率 |
```

### Source 类型

**渲染建议**：
- 每个原文片段使用卡片样式
- 文件名作为卡片标题
- 位置信息使用小号灰色文字
- 原文内容使用引用块样式
- 支持点击跳转到原文（如果有行号信息）

```markdown
## 原文来源

### 📄 neural-network.md

**位置**: 第2章 > 2.1节 (行 15-20)

> 神经元是神经网络的基本处理单元，它接收输入信号，
> 进行加权求和，然后通过激活函数产生输出。

---
```

### Relations 类型

**渲染建议**：
- 按关系类型分组显示（前置、相关、对比、相似）
- 知识点 ID 使用链接样式
- 点击链接跳转到对应知识点
- 描述信息使用小号字体

```markdown
## 知识关联

### 🔸 前置知识

- [[kp_000]]
  - 线性代数基础

### 🔗 相关知识

- [[kp_002]]
  - 激活函数详解
```

---

## 知识点跳转

### kpLinks 字段格式

```json
{
  "text": "激活函数",
  "targetKpId": "kp_002",
  "relationship": "related",
  "targetTitle": "激活函数详解"
}
```

### 前端处理建议

1. **渲染时**：识别 `content` 中的 `[[kpId]]` 格式，替换为可点击链接
2. **点击时**：
   - 保存当前浏览位置到历史栈
   - 调用 `/slides?kpId=kp_002` 获取目标知识点幻灯片
   - 替换当前显示内容

```typescript
// 渲染知识点链接
function renderContentWithLinks(content: string, kpLinks: KpLink[]): string {
  let html = content;
  kpLinks.forEach(link => {
    const pattern = new RegExp(`\\[\\[${link.targetKpId}\\]\\]`, 'g');
    html = html.replace(pattern, `<a href="#" class="kp-link" data-kp-id="${link.targetKpId}">${link.targetTitle || link.targetKpId}</a>`);
  });
  return html;
}

// 处理点击
function handleKpLinkClick(kpId: string) {
  // 保存当前状态
  pushHistory({ kpId: currentKpId, slideIndex: currentSlideIndex });
  // 加载新知识点
  loadSlides(kpId);
}
```

---

## 错误响应

| HTTP Code | Error Code | 说明 |
|----------|-----------|------|
| 400 | BAD_REQUEST | 请求参数错误 |
| 404 | KP_NOT_FOUND | 知识点不存在 |
| 500 | INTERNAL_ERROR | 服务器内部错误 |

```json
{
  "error": {
    "code": "KP_NOT_FOUND",
    "message": "知识点不存在: kp_001"
  }
}
```

---

## 注意事项

1. **内容为 Markdown 格式**：前端需要使用 Markdown 渲染器
2. **幻灯片数量不固定**：根据知识点实际内容生成，可能有 3-7 张
3. **知识点链接需要前端处理**：`[[kpId]]` 格式需要替换为可点击链接
4. **时间估算**：`estimatedTime` 可用于显示进度条或阅读时间提示
5. **难度等级**：根据 `Importance` 字段计算，可用于个性化推荐

---

## 与旧接口的兼容性

| 旧接口 | 新接口 | 说明 |
|--------|--------|------|
| `/overview` | `/slides` (type=cover) | 新接口的 cover slide 包含 overview 内容 |
| `/detailed-content` | `/slides` (type=explanation/detail/deepdive) | 新接口分三张幻灯片展示 |
| `/source-content` | `/slides` (type=source) | 新接口的 source slide |

旧接口仍然可用，但建议逐步迁移到新接口。
