...
| **Relations** | Relations | 知识关联 | 与其他知识点的关联，基于知识图谱的语义关系 |
...
- **IKnowledgeGraphBuilder**：用于获取知识点的知识图谱关联信息，包括邻居节点和关系类型。
...
2. **知识图谱关联准备**：
   - 调用 `IKnowledgeGraphBuilder` 获取当前知识点的知识图谱关联信息。
   - 获取当前知识点的邻居节点和关联关系，包括包含、依赖、相关、对比等关系类型。
   - 计算关联知识点的重要性和权重，用于生成 Relations 幻灯片。
...
3. **核心生成**：
   ...
   - 生成的内容结构包括：
     ...
     - `slide_cards`：包含多种类型的幻灯片卡片，每张卡片包含 `speechScript` 字段用于 TTS。
     - `knowledge_graph_relations`：包含当前知识点的图谱关联信息，用于生成 Relations 幻灯片。

4. **后处理与增强**：
   - 将生成的 DTO 转换为领域模型，如 `LearningPack` 和 `SlideCard`。
   - 转换幻灯片卡片类型，确保与系统定义的类型一致。
   - 为幻灯片卡片添加原文引用信息，包括文件路径、章节路径、行号范围等。
   - 为 Relations 幻灯片添加知识图谱关联信息，包括关联知识点的标题、类型、关系描述和权重。

5. **降级策略**：
   - 如果输入文本长度不足或生成失败，使用 `CreateFallbackLearningPack` 方法生成简化版学习内容。
   - 从原文片段中提取要点，填充基本信息。
   - 如果知识图谱数据不可用，生成基础的 Relations 幻灯片，不包含详细的关联分析。
...
```json
{
  ...
  "slideCards": [
    ...
    {
      "slideId": "slide_relations_001",
      "kpId": "kp_001",
      "type": "relations",
      "order": 5,
      "title": "知识关联",
      "htmlContent": "<div class='relations'>...</div>",
      "knowledgeGraphData": {
        "nodeId": "kp_001",
        "neighbors": [
          {
