...
**组合式函数 / store（建议）**
- `useBookHubs()`：加载 `/books/hubs`、激活 `/books/activate`
- `useScanTask()`：触发 `/books/scan`、轮询（若后端提供 taskId 查询；未提供则用"粗粒度 loading"）
- `useAppContextStore()`：当前 bookHubId、scanStatus、lastError
