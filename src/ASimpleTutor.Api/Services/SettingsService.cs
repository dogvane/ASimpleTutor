...
public class SettingsService : ISettingsService
{
    ...
    public async Task<LlmSettingsResponse> UpdateLlmSettingsAsync(LlmSettingsRequest request)
    {
        ...
        _config.Llm.Concurrency = request.Concurrency;

        // 3. 实时更新 LLM 服务的配置
        _llmService.UpdateConfig(request.ApiKey, request.BaseUrl, request.Model, request.Concurrency);

        // 4. 持久化到 appsettings.user.json
        await SaveUserConfigAsync();

        _logger.LogInformation("LLM 配置已更新并实时生效: {BaseUrl}, {Model}, 并发数: {Concurrency}",
            request.BaseUrl, request.Model, request.Concurrency);
    ...
    /// 验证 LLM 请求参数
    /// </summary>
    private void ValidateLlmRequest(LlmSettingsRequest request)
    {
        ...
        if (request.Concurrency < 1 || request.Concurrency > 10)
            throw new ArgumentException("并发数必须在 1-10 之间", nameof(request.Concurrency));
    }

    /// <summary>
    /// 获取当前 TTS 配置（API Key 敏感）
    /// </summary>
    ...
    private void ValidateRequest(TtsSettingsRequest request)
    {
        ...
        if (request.Speed < 0.25f || request.Speed > 4.0f)
            throw new ArgumentException("Speed 必须在 0.25-4.0 之间", nameof(request.Speed));
    }
